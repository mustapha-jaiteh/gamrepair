...
export default function AdminLayout({ children }) {
    ...
    const navItems = [
        { name: 'Dashboard', href: route('admin.dashboard'), icon: LayoutDashboard, routeName: 'admin.dashboard' },
        { name: 'Users', href: route('admin.users'), icon: Users, routeName: 'admin.users' },
        { name: 'Mechanics', href: route('admin.mechanics'), icon: Wrench, routeName: 'admin.mechanics' },
        { name: 'Bookings', href: route('admin.bookings'), icon: CalendarCheck, routeName: 'admin.bookings' },
        { name: 'Services', href: route('admin.services'), icon: ClipboardList, routeName: 'admin.services' },
        { name: 'Feedbacks', href: route('admin.feedbacks'), icon: MessageSquare, routeName: 'admin.feedbacks' },
        { name: 'System Logs', href: route('admin.logs'), icon: Activity, routeName: 'admin.logs' },
    ];

    const SidebarContent = () => (
        <>
            <div className="p-6 flex items-center gap-3 border-b border-slate-800 group cursor-pointer">
            ...
            <main className="flex-1 md:ml-64 w-full overflow-x-hidden">
                <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-10 w-full">
                    ...
                    <div className="flex items-center gap-2 md:gap-4">
                        <div className="p-2 bg-slate-100 rounded-full hover:bg-slate-200 cursor-pointer transition-colors relative hidden sm:block">
