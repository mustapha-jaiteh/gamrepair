...
export default function UserLayout({ children }) {
    ...
    const navigation = [
        ...
        },
        {
            ...
            icon: Wrench,
            routeName: "user.mechanics",
        },
        {
            name: "My Bookings",
            href: route("user.bookings"),
            icon: History,
            routeName: "user.bookings",
        },
        {
            name: "Profile",
    ...
    const SidebarContent = ({ isMobile = false }) => (
        <div className="flex flex-col h-full px-4 py-8">
            {/* Brand */}
            <div
                className={`flex items-center gap-4 mb-12 px-2 ${!isSidebarOpen && !isMobile ? "justify-center" : ""}`}
            >
                ...
                {(isSidebarOpen || isMobile) && (
                    <span className="text-xl font-black text-white tracking-tight">
                        ORVBA<span className="text-indigo-400">.</span>
                    </span>
                )}
            </div>
            ...
            <nav className="flex-1 space-y-2">
                {navigation.map((item) => {
                    ...
                    return (
                        ...
                        >
                            <item.icon
                                className={`w-5 h-5 transition-colors ${isActive ? "text-white" : "group-hover:text-slate-100"}`}
                            />
                            {(isSidebarOpen || isMobile) && (
    ...
    return (
        <div className="min-h-screen bg-[#FDFCFB] flex">
            ...
            {/* Mobile Sidebar */}
            <aside
                className={`fixed inset-y-0 left-0 w-72 bg-slate-900 text-slate-100 flex flex-col z-50 transform transition-transform duration-300 ease-in-out md:hidden ${
            ...
            {/* Desktop Sidebar */}
            <aside
                className={`hidden md:block fixed top-0 left-0 z-40 h-screen transition-all duration-500 ease-in-out border-r border-slate-800 bg-slate-900 text-slate-100 overflow-hidden ${isSidebarOpen ? "w-72" : "w-24"}`}
            >
                <SidebarContent />
            </aside>
            ...
            {/* Main Content */}
            <main
                className={`flex-1 transition-all duration-500 ease-in-out w-full overflow-x-hidden ${isSidebarOpen ? "md:ml-72" : "md:ml-24"}`}
            >
                {/* Header */}
