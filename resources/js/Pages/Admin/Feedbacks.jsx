        ...
        <AdminLayout>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-3">
                    ...
                    <input 
                        type="text" 
                        placeholder="Search feedback by username or content..." 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {feedbacks.map((item) => (
                        <div key={item.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 group relative p-6 flex flex-col">
                            ...
                            <div className="flex items-center gap-3 mb-6">
                                <div className="w-10 h-10 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-600 font-bold border border-slate-200">
                                    {item.user_name?.charAt(0) || 'U'}
                                </div>
                                <div>
                                    <h3 className="font-bold text-slate-900 leading-none">{item.user_name || 'Anonymous User'}</h3>
                                    <div className="flex items-center gap-1.5 mt-1">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <Star key={star} className={`w-3 h-3 ${star <= (item.rating || 5) ? 'text-orange-400 fill-orange-400' : 'text-slate-200'}`} />
                                        ))}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex-1">
                                <p className="text-slate-600 text-sm leading-relaxed italic">
                                    "{item.content || item.comment || 'No comment provided.'}"
                                </p>
                            </div>
                    ...
                    {feedbacks.length === 0 && (
                        <div className="col-span-full py-20 text-center bg-white rounded-4xl border border-dashed border-slate-200">
                            ...
                            <h3 className="text-slate-900 font-bold text-lg">No feedback yet</h3>
                            <p className="text-slate-400 text-sm">Feedback from users will appear here once services are completed.</p>
                        </div>
                    )}
                </div>
            </div>
